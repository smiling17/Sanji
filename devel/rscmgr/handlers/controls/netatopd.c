...
static void print_info(struct netpertask *np);
...
//
	...
	*/
#endif
//		fprintf(stderr, "checkpoint 1-3\n");
	histfd = histopen(&nap);

	/*
	** continuously obtain the info about exited processes
	*/
//		fprintf(stderr, "checkpoint 1-4\n");
	int count = 0;	
	for (EVER)
	{
		/*
...
//		fprintf(stderr, "checkpoint 2-1\n");
		...
		fprintf(stdout, "Check process statistic : sshd -> %d\n", ++count);
...
//		fprintf(stderr, "checkpoint 2-3 -> ret : %d\n", ret);
			print_info(&npt);
		switch (ret)
		{
		   case 0:
		   ...
		   default:	// getsockopt failed
			switch (errno)
			{
			   ...
			   default:
				fprintf(stdout, "default errno : %d -> %s.. continue\n", errno, strerror(errno));
				sleep(3);
				continue;
...
static void print_info(struct netpertask *np) {
	fprintf(stdout, "*****task info start*****\n");
	fprintf(stdout, "id : %d\n", np.id);
	fprintf(stdout, "btime : %d\n", np.btime);
	fprintf(stdout, "cmd : %s\n", np.command);
	fprintf(stdout, "tcpsndpacks : %d\n", np.tc.tcpsndpacks);
	fprintf(stdout, "tcpsndbytes : %d\n", np.tc.tcpsndbytes);
	fprintf(stdout, "tcprcvpacks : %d\n", np.tc.tcprcvpacks);
	fprintf(stdout, "tcprcvbytes : %d\n", np.tc.tcprcvbytes);
	fprintf(stdout, "udpsndpacks : %d\n", np.tc.udpsndpacks);
	fprintf(stdout, "udpsndbytes : %d\n", np.tc.udpsndbytes);
	fprintf(stdout, "udprcvpacks : %d\n", np.tc.udprcvpacks);
	fprintf(stdout, "udprcvbytes : %d\n", np.tc.udprcvbytes);
	fprintf(stdout, "***** task info end*****\n");
}
