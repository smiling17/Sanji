...
int main(int argc, char *argv[]){
	...
	if (bind(nl_sock, (struct sockaddr*)&bind_nladdr, sizeof(bind_nladdr)) < 0) {
	...
	int rev = epoll_ctl(epoll, EPOLL_CTL_ADD, nl_sock, &evt);
	if (rev == -1) {
		/* error 처리 */
		return -1;
	}
	...
	while(1){
	  int cnt = epoll_wait(epoll, evts, sizeof(evts) / sizeof(*evts), -1);
	  if (evt.data.fd != nl_sock) {
		  continue;
	  }
	  fprintf(stdout, "Invoke event...\n");
